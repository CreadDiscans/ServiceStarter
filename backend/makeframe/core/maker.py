...
class Maker:
  ...
  REACT_DATA_TEST = '''import DataTestFrame from '../DataTestFrame';\nclass DataTest%s extends DataTestFrame {
  componentWillMount() {
    this.url = '%s/%s/';
    this.title = '%s %s';
    this.setState({
  ...
  def makeReact(self, models, name):
    ...
    for klass in models:
      ...
      self.makeReactDataTest(instance, directory, name)
  ...
  def makeReactDataTest(self, instance, directory, appName):
    name = 'DataTest'+instance.__class__.__name__
    lowername = instance.__class__.__name__.lower().replace(appName, '')
